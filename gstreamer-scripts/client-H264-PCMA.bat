...
set VIDEO_CAPS=application/x-rtp,media=(string)video,clock-rate=(int)90000,encoding-name=(string)H264
set AUDIO_CAPS=application/x-rtp,media=(string)audio,clock-rate=(int)8000,encoding-name=(string)PCMA

set VIDEO_DEC=rtph264depay ! avdec_h264
set AUDIO_DEC=rtppcmadepay ! alawdec

set VIDEO_SINK=videoconvert ! autovideosink
set AUDIO_SINK=audioconvert ! audioresample ! autoaudiosink
