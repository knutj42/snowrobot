...
set VELEM=ksvideosrc
::VELEM="videotestsrc is-live=1"
set VCAPS=video/x-raw,width=640,height=360,framerate=15/1
set VSOURCE=%VELEM% ! queue ! videorate ! videoconvert ! %VCAPS%
set VENC=x264enc tune=zerolatency byte-stream=true bitrate=300 ! rtph264pay

set VRTPSINK=udpsink port=5000 host=%DEST% ts-offset=%VOFFSET% name=vrtpsink
set VRTCPSINK=udpsink port=5001 host=%DEST% sync=false async=false name=vrtcpsink
set VRTCPSRC=udpsrc port=5005 name=vrtpsrc

:: PCMA encode from the source
set AELEM=autoaudiosrc
::AELEM="audiotestsrc is-live=1"
set ASOURCE=%AELEM% ! queue ! audioresample ! audioconvert
set AENC=alawenc ! rtppcmapay

set ARTPSINK=udpsink port=5002 host=%DEST% ts-offset=%AOFFSET% name=artpsink
set ARTCPSINK=udpsink port=5003 host=%DEST% sync=false async=false name=artcpsink
set ARTCPSRC=udpsrc port=5007 name=artpsrc
